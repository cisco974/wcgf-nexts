...
const RankingChart: React.FC<RankingChartProps> = ({
    ...
    useEffect(() => {
        ...
        const chart = new Chart(canvasRef.current, { id, series, colors, height, animate });
...
class Chart {
    ...
    private drawGrid() {
        ...
        for (let i = 0; i <= gridLines; i++) {
            const value = i * step;
            const y = this.height - this.bottomMargin - (value / this.maxValue) * (this.height - this.margin - this.bottomMargin);
    ...
    private drawXAxisLabels() {
        ...
        this.labels.forEach((label, index) => {
            const x = this.margin + (index / (this.labels.length - 1)) * (this.canvas.width - 2 * this.margin);
    ...
    private drawSeries(serie: ChartSeries, color: string) {
        ...
        serie.data.forEach((point, index) => {
            const x = this.margin + (index / (serie.data.length - 1)) * (this.canvas.width - 2 * this.margin);
            const y = this.height - this.bottomMargin - (point.value / this.maxValue) * (this.height - this.margin - this.bottomMargin);
